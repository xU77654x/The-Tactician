...
public abstract class TacticianRelic extends CustomRelic {
    ...
    public void loadLargeImg() {
        if (notPng(imageName)) { if (largeImg == null) { this.largeImg = ImageMaster.loadImage(relicPath("large/" + imageName + ".png")); }}
        else { super.loadLargeImg(); }
    }

    private void setPool(AbstractCard.CardColor pool) {
        switch (pool) { // Basegame pools are handled differently
            case RED: relicType = RelicType.RED; break;
            case GREEN: relicType = RelicType.GREEN; break;
            case BLUE: relicType = RelicType.BLUE; break;
            case PURPLE: relicType = RelicType.PURPLE; break;
            default: this.pool = pool; break;
        }
    }
    ...
    private static String testStrings(String ID) {
        ...
        if (text == null) { throw new RuntimeException("The \"" + ID + "\" relic does not have associated text. Make sure there's no issue with the RelicStrings.json file, and that the ID in the json file matches the relic's ID. It should look like \"${modID}:" + GeneralUtils.removePrefix(ID) + "\"."); }
        return ID;
    }

    private static boolean notPng(String name) { return !name.endsWith(".png"); }
}