...
public class ArcfireBurstParticleEffect extends AbstractGameEffect {
	...
	public ArcfireBurstParticleEffect(float x, float y) {
		...
		else { this.img = ImageMaster.TORCH_FIRE_3; }
		this.duration = MathUtils.random(0.5F, 1.0F);
		this.x = x - ((float) this.img.packedWidth / 2);
		this.y = y - ((float) this.img.packedHeight / 2);
		this.color = new Color(MathUtils.random(0.9F, 1.0F), MathUtils.random(0.4F, 0.6F), MathUtils.random(0.1F, 0.3F), 0.0F);
		this.color.a = 0.0F;
	...
	public void update() {
		...
		if (this.scale > 0.3F * Settings.scale) { this.scale -= Gdx.graphics.getDeltaTime() * 2.0F; }
		if (this.y < this.floor) {
			this.vY = -this.vY * 0.75F;
			this.y = this.floor + 0.1F;
			this.vX *= 1.1F;
		}
		if (1.0F - this.duration < 0.1F) { this.color.a = Interpolation.fade.apply(0.0F, 1.0F, (1.0F - this.duration) * 10.0F); }
		else { this.color.a = Interpolation.pow2Out.apply(0.0F, 1.0F, this.duration); }
