...
public class Weapon1SwordPower extends AbstractPower {
	public static final String POWER_ID = TacticianMod.makeID("Weapon1SwordPower");
	private static final Texture tex84 = TextureLoader.getTexture(powerPath("large/Weapon1Sword_Large.png"));
	private static final Texture tex32 = TextureLoader.getTexture(powerPath("Weapon1Sword.png"));
	private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
	public static final String NAME = powerStrings.NAME;
	public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;
	public AbstractPlayer p;

	public Weapon1SwordPower(AbstractCreature owner) {
		this.name = NAME;
		this.ID = POWER_ID;
		this.owner = owner;
		this.amount = -1;
	...
	public float atDamageGive(float damage, DamageInfo.DamageType type) {
		if (type == DamageInfo.DamageType.NORMAL) {
			if (owner != this.p) {
				...
				else if (this.p.hasPower(Weapon2LancePower.POWER_ID) || this.p.hasPower(Weapon4BowPower.POWER_ID)) { return damage -3; }
				else if (this.p.hasPower(Weapon3AxePower.POWER_ID) || this.p.hasPower(Weapon8DarkPower.POWER_ID)) { return damage +3; }
			}
		}
		return damage;
	}
