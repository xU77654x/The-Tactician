...
public class HandSelectAction extends AbstractGameAction {
    ...
    private Consumer<ArrayList<AbstractCard>> process;
    private Consumer<ArrayList<AbstractCard>> finale;
    ...
    public void update() {
        if (this.first) {
            ...
            if (AbstractDungeon.player.hand.isEmpty()) {
                if (this.finale != null) { this.finale.accept(new ArrayList<>()); }
                this.isDone = true;
                return;
            }
            for (AbstractCard c : AbstractDungeon.player.hand.group) { if (this.requirement.test(c)) { validCards.add(c); } }
            ...
            if (validCards.size() <= this.amount && !this.anyAmount) {
                if (!this.returnSelected) { AbstractDungeon.player.hand.group.removeAll(validCards); }
                this.process.accept(validCards);
                if (this.finale != null) { this.finale.accept(validCards); }
                this.isDone = true;
                return;
            }
            this.invalidCards.addAll(AbstractDungeon.player.hand.group);
            Objects.requireNonNull(validCards);
            this.invalidCards.removeIf(validCards::contains);
            AbstractDungeon.player.hand.group.removeAll(this.invalidCards);
            if (AbstractDungeon.player.hand.isEmpty()) {
                returnCards();
                if (!this.returnSelected) { AbstractDungeon.player.hand.group.removeAll(validCards); }
                if (this.finale != null) { this.finale.accept(validCards); }
        ...
        if (!AbstractDungeon.handCardSelectScreen.wereCardsRetrieved) {
            ...
            if (this.returnSelected) { for (AbstractCard c : AbstractDungeon.handCardSelectScreen.selectedCards.group) { AbstractDungeon.player.hand.addToTop(c); } }
            returnCards();
            AbstractDungeon.handCardSelectScreen.wereCardsRetrieved = true;
        }
        if (this.finale != null) { this.finale.accept(result); }
        this.isDone = true;
    }

    private void returnCards() {
        for (AbstractCard c : this.invalidCards) { AbstractDungeon.player.hand.addToTop(c); }
        AbstractDungeon.player.hand.refreshHandLayout();
    }
}

// Code directly taken from Downfall: Hermit.