...
public class Weapon2LancePower extends AbstractPower {
	...
	public Weapon2LancePower(AbstractCreature owner) {
		this.name = NAME;
		this.ID = POWER_ID;
		this.owner = owner;
		this.amount = -1;
	...
	public float atDamageGive(float damage, DamageInfo.DamageType type) {
		if (type == DamageInfo.DamageType.NORMAL) {
			if (owner != this.p) {
				...
				else if (this.p.hasPower(Weapon3AxePower.POWER_ID) || this.p.hasPower(Weapon4BowPower.POWER_ID)) { return damage -3; }
				else if (this.p.hasPower(Weapon1SwordPower.POWER_ID) || this.p.hasPower(Weapon8DarkPower.POWER_ID)) { return damage +3; }
			}
		}
		return damage;
	}
