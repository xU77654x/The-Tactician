...
public class IgnisParticleEffect extends AbstractGameEffect {
	...
	public IgnisParticleEffect(float x, float y) {
		setImg();
		...
		this.color = new Color(1.0F, 1.0F, 1.0F, 0.45F);
		this.color.g -= MathUtils.random(0.5F);
		this.color.r -= this.color.g - MathUtils.random(0.5F);
	...
	public void update() {
		...
		// else if (this.duration > this.startingDuration / 2.0F) { this.color.a = Interpolation.fade.apply(0.0F, 0.5F, (this.startingDuration - this.duration) / this.startingDuration / 2.0F); }
		// else if (this.duration < this.startingDuration / 2.0F) { this.color.a = Interpolation.fade.apply(0.0F, 0.5F, this.duration / this.startingDuration / 2.0F); }
	}

	private void setImg() {
		int roll = MathUtils.random(2);
		if (roll == 0) { this.img = ImageMaster.FLAME_1; }
		else if (roll == 1) { this.img = ImageMaster.FLAME_2; }
		else { this.img = ImageMaster.FLAME_3; }
	}

	public void render(SpriteBatch sb) {
		sb.setColor(this.color);
		sb.setBlendFunction(770, 1);
		if (this.flipX && !this.img.isFlipX()) { this.img.flip(true, false); }
		else if (!this.flipX && this.img.isFlipX()) { this.img.flip(true, false); }
